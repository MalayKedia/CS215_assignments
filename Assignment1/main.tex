...
\begin{align*}
    \frac{P(E_1)}{P(E_2)} = \frac{1+a}{1+b} \leq 1, \hspace{40 mm} \{\because \hspace{2 mm}b \geq a\}
\end{align*}
Showing that the second scenario is more favourable for our team.

\end{solution}

\question{\bf Random Variables}
...
\begin{align*}
    \hspace{10 mm} P(Q_1 \geq q_1) \leq p_1,  P(Q_2 \geq q_2) \leq p_2
\end{align*}

First let's find $P(Q_1\cdot Q_2 \geq q_1\cdot q_2)$ :
\begin{align*}
    P(Q_1\cdot Q_2 \geq q_1\cdot q_2) &\leq P(Q_1 \geq q_1 \vee Q_2 \geq q_2)
    \\&\leq P(Q_1 \geq q_1) + (Q_2 \geq q_2)
    \\&\leq p_1 + p_2
\end{align*}
...
\question{\bf Staff Assistant}

\begin{solution}\\
(a) $E_i$ is the event that $i^{th}$ candidate is the best and we hire him. For this to happen, two conditions have to be met:
\begin{enumerate}
    \item $i^{th}$ candidate is the best among all the n appeared (Event $C_{1}$), the probability of which is given by :
    \[ P(C_1)=\frac{1}{n} \text{     (All are equally likely to be the best)} \]
    
    \item  None of candidates $m+1, m+2, ..., i-1$ are picked (Event $C_{2}$), the probability of which is given by:
    \[
    P(C_{2})=\frac{m}{i-1}
    \]
    as the maximum of $1, 2, ..., i-1$ must lie in $1, 2, ..., m$ for the condition to hold.
\end{enumerate}
...
\[
\begin{aligned}
    Pr(E_{i})=&P(C_{1} \cap C_{2})\\
   =&P(C_{1})P(C_{2}|C_{1})\\
    =&P(C_{1})P(C_{2}) \hspace{40 mm} \text{as $C_2$ and $C_1$ are independant}\\
    =&\frac{m}{n(i-1)}
\end{aligned}
\]

\[
\therefore Pr(E_i)=\begin{cases} 
0 & \text{if } i \leq m\\
\frac{m}{n(i-1)} & \text{if } i > m
\end{cases}
\]
Now, to determine the total probability that we hired the best candidate, sum over $Pr(E_{i})$ from $i=0$ to $i=n$:
\[
Pr(E)=\sum_{i=0}^{n}{Pr(E_{i})}
\]
\[
\Rightarrow Pr(E)=\sum_{i=0}^{m}{Pr(E_{i})} +\sum_{i=m+1}^{n}{Pr(E_{i})}
\]
\[
...
\newline
\[
\Rightarrow \frac{m}{n}[ln\vert \text{x}\vert]_{m}^{n} \leq P{r}(E) \leq \frac{m}{n}[ln\vert \text{x-1}\vert]_{m}^{n}
\]
\newline
...
\[
\begin{aligned}
F(r) &= P\left(\text{I win} \mid \text{I am at position } r\right) \\
&= P\left(\text{Traders at } 1,\ 2,\ \ldots,\ r-1\ \text{lose}\right) \times P\left(\text{I win at position } r\right)\\
\therefore F(r) &=
\begin{cases} 
0 & \text{if } r = 1, \\
...
\]
\[
\Rightarrow \sum_{i=0}^{n-1} {x_{i}} = \mu n
\]
Hence,
\[
\Rightarrow \mu_{new} = \frac{\sum_{i=0}^{n} {x_{i}}}{n+1}
=\frac{\sum_{i=0}^{n-1} {x_{i}} + x_{new}}{n+1}
\]
\[
...
\frac{OldMedian+A[\frac{n+1}{2}]}{2} & \text{if n is odd}.
\end{cases}
\]
(c) By the definition of standard Deviation,
\[
\sigma=\sqrt{\frac{\sum_{i=0}^{n-1}{(x_{i}-\mu)^2}}{n-1}}
\]
\[
\Rightarrow(n-1)\sigma^2 = \sum_{i=0}^{n-1}{(x_{i}-\mu)^2}
\]
\[
\Rightarrow(n-1)\sigma^2 = \sum_{i=0}^{n-1}{x_{i}^2} - 2\mu\sum_{i=0}^{n-1}{x_{i}} + n\mu^2
\]
\[
\Rightarrow(n-1)\sigma^2 = \sum_{i=0}^{n-1}{x_{i}^2} -n\mu^2
\]
\[
\Rightarrow \sum_{i=0}^{n-1}{x_{i}^2} = (n-1)\sigma^2 + n\mu^2
\]
\[
    \Rightarrow \sum_{i=0}^{n}{x_{i}^2} = (n-1)\sigma^2 + n\mu^2 + x_{new}^2
\]
\[
    \Rightarrow \sum_{i=0}^{n}{x_{i}^2} - (n+1)\mu_{new}^2 = (n-1)\sigma^2 + n\mu^2  + x_{new}^2 - (n+1)\mu_{new}^2
\]
\[
...
\begin{solution}\\
It is used to compare distribution of numeric data from one or more groups using the density curves. Wider curve represents a higher frequency of data points in that region. \\ \\It provides a more detailed analysis than a normal box plot giving a better view of the distribution and skewness  of the graph.(Skewness refers to the asymmetric distribution around the mean of data)
...
\begin{center}
    \includegraphics[width=0.7\linewidth]{coxcomb.png}
    \caption{\\Coxcomb plot showing the number of offline stores for a company in 3 different countries at 2 different times}
    \label{fig:coxcomb}
\end{center}
\end{solution}
...
\begin{center}
    \includegraphics[width=1\linewidth]{spectro.png}
...
\begin{center}
    \includegraphics[width=1\linewidth]{waterfall_plot.png}
...
\question{\bf Mona Lisa}
\begin{solution}\\
    \textbf{Correlation Coefficient Plot}
    \begin{center}
    \includegraphics[width=0.8\linewidth]{correlation_plot.png}
    \caption{\\A plot of correlation coefficient across the shift values}
    \label{fig:correlation}
\end{center}

   \textbf{Normalised Histogram plot}
     \begin{center}
    \includegraphics[width=1\linewidth]{normalised_histograms.png}
    \caption{A normalised histogram of original image}
    \label{fig:histogram}
\end{center}
\end{solution}
