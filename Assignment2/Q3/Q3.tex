...
\[\mu_{1}^{bin} = E(X) = \sum_{x=0}^{n} x \binom{n}{x} p^x q^{n-x} \]
\[\mu_{1}^{bin}= np \sum_{x=1}^{n} \binom{n-1}{x-1} p^{x-1} q^{n-x} \]
\[\mu_{1}^{bin}= np (q + p)^{n-1}\]
\[\mu_{1}^{bin}= np\]
Second moment of Binomial Distribution, $\mu_{2}^{bin}$
\[\mu_{2}^{bin} = E(X^2) = \sum_{x=0}^{n} x^2 \binom{n}{x} p^x q^{n-x} \]
\[\mu_{2}^{bin}= \sum_{x=0}^{n} \left\{x(x-1) + x\right\} \binom{n}{x} \binom{n-1}{x-1} \binom{n-2}{x-2} p^x q^{n-x} \]
\[\mu_{2}^{bin}= n(n-1)p^2 \left\{\sum_{x=2}^{n} \binom{n-2}{x-2} p^{x-2} q^{n-x}\right\} + np \]
\[\mu_{2}^{bin}= n(n-1)p^2 (q + p)^{n-2} + np \]
...
\begin{center}
    \includegraphics[width=0.75\linewidth]{3e.png}
    \caption{\\GMM Fitting}
\end{center}
\end{solution}
\textbf{Teaser (Ungraded Bonus)}
\begin{solution}
